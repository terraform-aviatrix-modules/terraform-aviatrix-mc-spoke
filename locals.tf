locals {
  ...
  additional_group_mode_subnets_map = {
    aws = local.cloud == "aws" ? (var.use_existing_vpc ? [] : try(slice(aviatrix_vpc.default[0].public_subnets.*.cidr, 2, length(aviatrix_vpc.default[0].public_subnets)), [])) : null #Get the rest of the public subnets, minus the first 2. Wrapped in try, in case there are less than 2 subnets in total.
  }
